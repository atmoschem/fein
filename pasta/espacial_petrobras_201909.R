...
dt[anoReferencia == 2018 , 
   sum(emi_g, na.rm = T), 
   by = .(LAT_DEC, LONG_DEC, Pollutant, mesReferencia)] -> biomassas
...
dt[anoReferencia == 2018, 
   sum(emi_g, na.rm = T), 
   by = .(LAT_DEC, LONG_DEC, Pollutant, mesReferencia)] -> carvao
...
dt[anoReferencia == 2018, 
   sum(emi_g, na.rm = T), 
   by = .(LAT_DEC, LONG_DEC, Pollutant, mesReferencia)] -> gas_natural
...
dt[anoReferencia == 2018, 
   sum(emi_g, na.rm = T), 
   by = .(LAT_DEC, LONG_DEC, Pollutant, mesReferencia)] -> oleo_diesel
...
dt[anoReferencia == 2018, 
   sum(emi_g, na.rm = T), 
   by = .(LAT_DEC, LONG_DEC,  Pollutant, mesReferencia)] -> proce_indus
...
dt[year == 2018, 
   sum(g, na.rm = T), 
   by = .(lat, lon, Pollutant, month)] -> refinaria
...
dt[year == 2018, 
   sum(emi_g, na.rm = T), 
   by = .(Latitude, Longitude,  Pollutant, month)] -> cemento
...
dt[year == 2018, 
   sum(g, na.rm = T), 
   by = .(lat, lon, Pollutant, month)] -> pulpa
...
fwrite(dt, "../../BRA_sp/petrobras/2018_industry/industry_2018.csv.gz")


library(data.table)
library(sf)
library(cptcity)
dt <- fread("../../BRA_sp/petrobras/2018_industry/industry_2018.csv.gz")
yy <- dt[mesReferencia == 9 & 
     Pollutant == "SOx",
   sum(V1), 
   by = .(LAT_DEC, LONG_DEC, Pollutant, type)]

yy[V1 == max(V1)]
3325383363/30/24
dt <- st_as_sf(dt[!is.na(LAT_DEC)], coords = c( "LONG_DEC", "LAT_DEC"), crs = 4326)
...
dx[Pollutant == "CO" & year == 2018, sum(V1), by = .(Pollutant, Estimation)]
# Pollutant Estimation            V1
# 1:        CO     Sergio  700995.6 [t]
# 2:        CO        Ana 7539727.6 [t]

dx[Pollutant == "NOx" & year == 2018, sum(V1), by = .(Pollutant, Estimation)]
# Pollutant Estimation            V1
# 1:       NOx     Sergio 4067183.0 [t]
# 2:       NOx        Ana  441498.2 [t]

dx[Pollutant == "NMVOC" & year == 2018, sum(V1), by = .(Pollutant, Estimation)]
# Pollutant Estimation           V1
# 1:     NMVOC     Sergio 185703.7 [t]
# 2:     NMVOC        Ana 222018.5 [t]


dx[Pollutant == "PM2.5" & year == 2018, sum(V1), by = .(Pollutant, Estimation)]
# Pollutant Estimation           V1
